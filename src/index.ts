...
async function generateRestaurantReply(conversationId: string, customerName: string): Promise<string | null> {
  try {
    ...
    const messages: any[] = [
      {
        role: "system",
        content: `أنت موظف خدمة عملاء في ${businessName}. اسم العميل: ${customerName || "العميل"}.
${businessInfo ? `معلومات: ${businessInfo}` : ""}

المنيو (هذه هي الأصناف الحقيقية الوحيدة، لا تخترع أصناف أو أسعار غير موجودة):
${menuText}

قواعد صارمة:
- رد بنفس لغة العميل فقط
- ردك جملة أو جملتين فقط، لا تكرر نفسك
- لا تخترع أصناف أو خصومات غير موجودة في المنيو أعلاه
- لو سأل عن خصم غير موجود، قل بصراحة إنه غير متاح حالياً
- لو أراد الطلب، اسأل فقط: الكمية؟ وعنوان التوصيل؟
